...
while {true} do {
	...
	sleep 1;
			...
			if (_lastchar >= 34 and _lastchar <= 45) then {["kat_circulation_bloodgroup", "O-"] call CBA_settings_fnc_set; [_unit, "USP_PATCH_BLOOD_ONEG_BLK"] remoteExec ["bis_fnc_setUnitInsignia", 0];};
			...
			if (_lastchar >= 74 and _lastchar <= 82) then {["kat_circulation_bloodgroup", "A-"] call CBA_settings_fnc_set; [_unit, "USP_PATCH_BLOOD_ANEG_BLK"] remoteExec ["bis_fnc_setUnitInsignia", 0];};
			...
			if (_lastchar >= 92 and _lastchar <= 94) then {["kat_circulation_bloodgroup", "B-"] call CBA_settings_fnc_set; [_unit, "USP_PATCH_BLOOD_BNEG_BLK"] remoteExec ["bis_fnc_setUnitInsignia", 0];};
			...
			if (_lastchar >= 98) then {["kat_circulation_bloodgroup", "AB-"] call CBA_settings_fnc_set; [_unit, "USP_PATCH_BLOOD_ABNEG_BLK"] remoteExec ["bis_fnc_setUnitInsignia", 0];};
